...
goto receiving_from${j}${sub}\n`);
	    }
	    fs.appendFileSync(write, `fi
after_enc${i}${sub}:// printf("process %d changed from after_enc to receiving\\n", ${i});
goto receiving_from${i}${sub};
after_dec${i}${sub}:if\n`);
	    for (var j = 0; j < no; j++) {
		fs.appendFileSync(write, `:: // printf("process %d changed from before_comm to after_enc\\n", ${i});
...
after_enc0${i}${sub}:if
:: `);
	    fs.appendFileSync(write, `// printf("process %d changed from after_enc0 to sending\\n", ${i});
goto sending_to${i}${sub}\n`)
	    for (var j = 0; j < no; j++) {
		fs.appendFileSync(write, `:: // printf("process %d changed from after_enc0 to receiving\\n", ${j});
goto receiving_from${j}${sub}\n`);
	    ...
	    fs.appendFileSync(write, `fi
after_enc${i}${sub}:`);
	    fs.appendFileSync(write, `// printf("process %d changed from after_enc to receiving\\n", ${i});
goto receiving_from${i}${sub};
after_dec${i}${sub}:if\n`);
	    for (var j = 0; j < no; j++) {
		fs.appendFileSync(write, `:: // printf("process %d changed from before_comm to after_enc\\n", ${i});
